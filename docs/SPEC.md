...
ferme sans papier** : la mesure « Vérifier le casque » (§6.2) — ou le bouton « Mesurer » posé à
côté du pic alpha sur la page SSVEP, qui ouvre la même mesure — rend le pic et propose de l'appliquer
...
Six mesures sont livrées. La première est une mesure de SÉANCE (une tuile sur l'accueil) ; les cinq
autres sont les **tests** des modes, ouverts par le bouton « Tester » de chaque page (2026-09-22,
chantier « Configurer · Entraîner · Tester » — un test est le protocole d'entraînement rejoué,
le moteur DÉCIDANT au lieu d'apprendre, sur les réglages courants du mode, sans rien écrire) :

| mesure | durée par défaut | rend | barrière ? |
|---|---|---|---|
| `alpha` — **Vérifier le casque** | 37 s | ratio yeux fermés / yeux ouverts, pic (Hz), verdict | **OUI** |
| `ssvep_taux` — **Tester le SSVEP** | 3,6 min | taux d'émission, justesse à l'émission (hasard 1/N) | non |
| `mi_test` — **Tester le Motor Imagery** | ≈ 2,8 min | taux d'émission, justesse à l'émission (hasard 1/nombre de classes) | non |
| `p300_test` — **Tester le P300** | ≈ 1,2 min | justesse de sélection par manche (hasard 1/6) | non |
| `cvep_test` — **Tester le c-VEP** | ≈ 1,8 min | justesse ET taux d'émission par bloc (hasard 1/6) | non |
| `errp_test` — **Tester l'ErrP** | ≈ 2,4 min | un COUPLE : bonnes commandes gardées / erreurs attrapées — un détecteur, pas de hasard unique | non |

⚠️ **UN ESSAI = UNE DÉCISION dans les cinq tests** : l'effectif porte sur des essais (manches,
blocs, feedbacks), jamais sur les fenêtres glissantes du moteur. Et chaque résultat s'affiche en
trois lignes — un MOT coloré décidé par le moteur, les chiffres avec leur point de comparaison, une
réserve —, le reste replié sous « Détails » (`core/modes/affichage.py`).
...
- Le **contrôle alpha est une BARRIÈRE** : sans montée d'alpha à la fermeture des yeux, les
  ...
  rien dire**. Le verdict ARRÊTE : le mot affiché en face, en rouge, est « ARRÊTE ICI » — un
  chiffre lu d'abord invite à négocier avec. Il remplace `research/alpha_check.py` (archivé) **à protocole
  identique** : les durées et les bandes font corps avec le repère « ratio > ~1,5 », donc la mesure
  n'expose **aucun** réglage.
- Le **test du SSVEP** (l'ancienne mesure « Taux d'émission SSVEP ») remplace
  `research/ssvep_guided.py`, un monolithe qui affichait,
     ...
     - **[fait 2026-08-19 — chantier « l'ErrP sur le réseau »]** l'**ErrP est publié**
       ...
       **modeste** : une erreur sur deux attrapée pour une bonne commande sur sept annulée, et il
       ...
       - **[fait 2026-09-07]** la **calibration ErrP est jouée par le moteur**
         (`core/modes/errp_calib.py`), lancée par le bouton « Entraîner » de la console. ⚠️ Elle est
