...
L'écran montre la commande décodée + les probabilités (utile pour progresser : tu vois l'effet).
    ...
    python src/mi_pilot.py                    # feedback seul (pas d'envoi robot) — pour s'entraîner
    python src/mi_pilot.py --send             # + pilote le robot (roues en l'air d'abord !)
    python src/mi_pilot.py --calibrate --send  # calibration PUIS pilotage, d'un coup
    python src/mi_pilot.py --calibrate --session 5min
    python src/mi_pilot.py --smoke            # test headless (CI)
"""
...
from config import (COMMANDS, MI_MIN_VOTES, MI_MODEL_PATH, MI_PROB_MIN, MI_VOTE_LEN,  # noqa: E402
                    MI_WINDOW_S, UDP_HOST, UDP_PORT, apply_invert, use_utf8_console)
