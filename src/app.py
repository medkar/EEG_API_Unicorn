"""Appli unifiée EEG -> Waffle : un menu, quatre modes de décodage, un seul robot.
    ...
    python src/app.py --send          # envoi UDP au robot armé dès le départ
...
from config import (ALPHA_PEAK_HZ, ARTIFACT_SIGMA_RATIO, BANDPASS, COMMANDS,  # noqa: E402
...
from itr import itr as _itr  # noqa: E402
from ssvep_stimulus import is_on as ssvep_on  # noqa: E402
from ui import (ACCENT, BAR_BG, BG, DIM, FG, GO, ON_COLOR, OUTLINE, WARN, Abort, App)  # noqa: E402
...
def mode_mi(app, model_path=MI_MODEL_PATH):
    from mi_decoder import MIDecoder, MIModel
    from mi_pilot import MIController
...
def mode_cvep(app, model_path=CVEP_MODEL_PATH):
    from cvep_code import build_targets, is_on as cvep_on
    from cvep_decoder import CVEPDecoder, CVEPModel
...
def mode_cvep_rcca(app, model_path=CVEP_RCCA_MODEL_PATH):
    ...
    from cvep_code import is_on as cvep_on
    from cvep_rcca import RCCADecoder, RCCAModel, build_targets_rcca
...
def mode_p300(app, model_path=P300_MODEL_PATH, dynamic=False):
    ...
    from p300_decoder import P300Model, epoch_from_stream
...
def _neuro_sample(app):
    ...
    from neuro_monitor import artifact_sigma, band_powers, emg_power, highpass_filter, indices_from_bp
...
def _neuro_baseline(app, seconds):
    ...
    from neuro_monitor import IndexNormalizer
...
def _errp_epoch(app, onset, fs):
    ...
    from p300_decoder import epoch_from_stream
...
def mode_errp(app, model_path=ERRP_MODEL_PATH):
    ...
    from errp_decoder import ERROR, ErrPModel
...
def page_errp(app):
    ...
    from errp_calibrate import adjust_threshold
    from errp_decoder import ErrPModel
...
def _smoke(app):
    ...
    import cvep_calibrate
    from mi_pilot import _dummy_model
