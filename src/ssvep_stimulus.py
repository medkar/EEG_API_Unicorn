...
Lancer :
    ...
    python src/ssvep_stimulus.py --windowed      # fenêtre 1000x700 (dev)
    python src/ssvep_stimulus.py --refresh 60    # forcer le refresh (sinon auto-mesuré)
    python src/ssvep_stimulus.py --seconds 20    # auto-quit après 20 s
    python src/ssvep_stimulus.py --smoke         # test sans écran (CI), n'affiche rien
"""
...
# Permet `from config import ...` que le module soit lancé via `python src/ssvep_stimulus.py`
# ou importé comme `src.ssvep_stimulus`.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from config import COMMANDS, COMMON_REFRESH, choose_frequencies, use_utf8_console  # noqa: E402
